...
<head>
    ...
    <title>Claddagh | Patrol Calendar</title>
    ...
    <style>
        ...
        .cal-day.has-postponed .cal-count-chip { background:rgba(209,213,219,.4); color:#4b5563; }
        ...
        .sel-action-btn.s-postpone { background:#e5e7eb; color:#374151; border-color:#d1d5db; }
        .sel-action-btn.s-postpone:hover:not(:disabled) { background:#d1d5db; }
        ...
        .manager-vol-card { background:#fff; border-radius:16px; box-shadow:0 2px 12px rgba(0,0,0,.07); overflow:hidden; margin-bottom:14px; }
        .manager-vol-header { padding:14px 16px; border-bottom:1px solid #f1f5f9; font-size:.95rem; font-weight:700; color:#1e3a5f; display:flex; align-items:center; justify-content:space-between; gap:8px; cursor:pointer; user-select:none; }
        .manager-vol-header:hover { background:#f8fafc; }
        .manager-vol-header-left { display:flex; align-items:center; gap:8px; }
        .manager-vol-toggle-icon { transition:transform .2s ease; color:#64748b; }
        .manager-vol-header[data-collapsed="false"] .manager-vol-toggle-icon { transform:rotate(180deg); }
        .manager-vol-body { border-top:1px solid #f1f5f9; }
        .manager-vol-body.d-none { display:none; }
        .manager-vol-list { padding:10px 14px 12px; max-height:360px; overflow:auto; }
        .manager-vol-item { border:1px solid #e2e8f0; border-radius:10px; padding:8px 10px; margin-bottom:8px; background:#f8fafc; display:flex; align-items:flex-start; justify-content:space-between; gap:8px; }
        .manager-vol-item:last-child { margin-bottom:0; }
        .manager-vol-content { flex:1; min-width:0; }
        .manager-vol-meta { font-size:.73rem; color:#64748b; margin-bottom:5px; display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
        .manager-vol-names { display:flex; flex-wrap:wrap; gap:6px; }
        .manager-vol-chip { font-size:.72rem; background:#e2e8f0; color:#334155; padding:3px 8px; border-radius:999px; }
        .manager-vol-actions { margin-top:8px; }
        .manager-vol-actions button { font-size:.72rem; padding:.22rem .6rem; }
        .manager-vol-empty { text-align:center; padding:18px 10px; color:#94a3b8; font-size:.82rem; }
...
<?php
    ...
    $__roleName  = $__payload['roleName'] ?? 'VOLUNTEER';
    $__role      = (int)($__payload['role'] ?? 99);
    $__isManager = in_array($__role, [1, 2], true); // ADMIN or MANAGER

    // -- Load schedules based on role -----------------------------------------
    if ($__isManager) {
        // Limit to 3 months back → future to keep the payload small
        $schedules = \App\Models\Schedule::with(['volunteers', 'supervisor'])
                      ->where('patrolDate', '>=', now()->subMonths(3)->startOfDay())
                      ->orderBy('patrolDate')
                      ->get();
    } else {
        // Volunteers see released (1), finalized (2), and postponed (3)
        $schedules = \App\Models\Schedule::with(['volunteers', 'supervisor'])
    ...
    if (!$__isManager) {
        // Volunteers: clamp to current month and next month only
        if ($qYear < $nowYear || ($qYear === $nowYear && $qMonth < $nowMonth)) {
            $qYear = $nowYear; $qMonth = $nowMonth;
        }
        if ($qYear > $nextYear || ($qYear === $nextYear && $qMonth > $nextMonth)) {
            $qYear = $nextYear; $qMonth = $nextMonth;
        }
    }
    $isFirstMonth = (!$__isManager && $qYear === $nowYear  && $qMonth === $nowMonth);
    $isLastMonth  = (!$__isManager && $qYear === $nextYear && $qMonth === $nextMonth);
    $calYear     = $qYear;
    $calMonth    = $qMonth;
    ...
    $schedulesJson = json_encode($schedules->values()->map(fn($s) => [
        ...
        'maxVolunteers'     => $s->maxVolunteers ?? null,
...
<div class="vol-layout-wrapper">
    ...
    <div class="vol-sidebar-col">
        ...
        <div class="manager-vol-card">
            <div class="manager-vol-header" id="managerVolHeaderToggle" data-collapsed="true" onclick="toggleManagerVolunteerMenu()">
                <div class="manager-vol-header-left">
                    <i class="bi bi-people-fill text-primary"></i>
                    Volunteers in Released/Finalized Patrols
                </div>
                <i class="bi bi-chevron-down manager-vol-toggle-icon"></i>
            </div>
            <div class="manager-vol-body d-none" id="managerVolunteerBody">
                <div class="manager-vol-list" id="managerVolunteerList">
                    <div class="manager-vol-empty">
                        <i class="bi bi-hourglass-split d-block mb-2 fs-4"></i>
                        Loading patrol volunteers...
                    </div>
                </div>
            </div>
        </div>
        ...
        <?php if ($__isManager): ?>
        <!-- Manager: Monthly Volunteer Statistics -->
        <div class="manager-vol-card" style="margin-top:14px;">
            <div class="manager-vol-header" id="statsHeaderToggle" data-collapsed="true" onclick="toggleMonthlyStats()">
                <div class="manager-vol-header-left">
                    <i class="bi bi-graph-up text-success"></i>
                    Monthly Signups
                    <span class="small text-muted" id="statsMonthLabel"></span>
                </div>
                <i class="bi bi-chevron-down stats-toggle-icon"></i>
            </div>
            <div class="manager-vol-body d-none" id="statsBody">
                <div id="volunteerStatsContainer" style="max-height:360px;overflow-y:auto;padding:14px 16px;">
                    <div class="text-muted small text-center py-3">Loading statistics…</div>
                </div>
            </div>
        </div>
        <?php endif; ?>
            ...
            <div class="upcoming-body d-none" id="upcomingBody">
                <div class="upcoming-list" style="padding-bottom:8px;">
                    <div class="filter-btns" id="filterBtns">
                        ...
                        <button class="filter-btn" onclick="setFilter('next', this)">Next Month</button>
                    </div>
                </div>
        ...
        <div class="legend-panel">
            ...
            <div class="legend-panel-body">
                <div class="legend-item">
                    ...
                    <span>Released &mdash; tap to volunteer</span>
                </div>
                <div class="legend-item">
                    <span class="legend-swatch" style="background:#dcfce7;border:1px solid #4ade80;"></span>
                    <span>Finalized</span>
                </div>
                <?php if ($__isManager): ?>
                <div class="legend-item">
...
<!---- Postpone Patrol Modal ----------------------------------------- -->
<div class="modal fade" id="postponePatrolModal" tabindex="-1" aria-modal="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:480px;">
        <div class="modal-content" style="border-radius:14px;">
            <div class="modal-header" style="background:#1e3a5f;color:#fff;border-radius:14px 14px 0 0;">
                <h5 class="modal-title"><i class="bi bi-clock-history me-2"></i>Postpone Patrol</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4">
                <div class="mb-1">
                    <label class="form-label fw-semibold">New Date</label>
                    <input type="date" class="form-control" id="postponeNewDate" required>
                </div>
            </div>
            <div class="modal-footer" style="background:#f8fafc;border-radius:0 0 14px 14px;">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-primary" onclick="submitPostponePatrol()">
                    <i class="bi bi-check-lg me-1"></i>Postpone
                </button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="requestVolunteerModal" tabindex="-1" aria-modal="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:560px;">
        <div class="modal-content" style="border-radius:14px;">
            <div class="modal-header" style="background:#1e3a5f;color:#fff;">
                <h5 class="modal-title"><i class="bi bi-megaphone-fill me-2"></i>Request Volunteers</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <p id="requestVolunteerInfo" class="mb-3"></p>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Need additional volunteers</label>
                    <input type="number" min="1" class="form-control" id="requestVolunteerShortage" value="1">
                    <div class="form-text">Manually enter the required number of people.</div>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Message type</label>
                    <select class="form-select" id="requestVolunteerMode" disabled>
                        <option value="individual">Individual selection</option>
                    </select>
                    <div class="form-text">Only individual messages are supported.</div>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Custom note (optional)</label>
                    <textarea id="requestVolunteerMessage" class="form-control" rows="3" placeholder="Enter note to include in email."></textarea>
                </div>
                <div id="requestVolunteerUserList" style="max-height:250px;overflow:auto;border:1px solid #e2e8f0;border-radius:8px;padding:8px;"></div>
                <div id="requestVolunteerSpinner" class="text-center d-none"><span class="spinner-border spinner-border-sm"></span> Sending…</div>
            </div>
            <div class="modal-footer" style="background:#f8fafc;border-radius:0 0 14px 14px;">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-primary" id="requestVolunteerSubmitBtn" onclick="sendRequestVolunteerEmails()">Send email request</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    const baseUrl = '<?php echo url('/volunteer'); ?>';
    ...
    let allVolunteers = []; // Store all volunteers for stats
    ...
    function openRequestVolunteerModal(patrolNr, defaultShortage) {
        const patrol = allSchedules.find(p => p.patrolNr === patrolNr);
        if (!patrol) {
            showToast('Patrol not found.', 'danger');
            return;
        }

        document.getElementById('requestVolunteerInfo').textContent = `Patrol #${patrolNr} on ${patrol.patrolDate} currently has ${getTeamCount(patrol)} volunteers.`;
        document.getElementById('requestVolunteerShortage').value = Math.max(1, defaultShortage);
        document.getElementById('requestVolunteerMode').value = 'individual'; // fixed individual mode only
        document.getElementById('requestVolunteerMessage').value = '';
        document.getElementById('requestVolunteerUserList').innerHTML = '<div class="text-muted small">Loading available volunteers…</div>';

        const candidateIds = new Set((patrol.volunteers || []).map(v => Number(v.id)));

        fetch('/api/users/volunteers')
            .then(res => res.json())
            .then(data => {
                if (!data.success) {
                    document.getElementById('requestVolunteerUserList').innerHTML = '<div class="text-danger small">Unable to fetch volunteers.</div>';
                    return;
                }
                const candidates = (data.users || []).filter(u => !candidateIds.has(u.UserNr));
                if (!candidates.length) {
                    document.getElementById('requestVolunteerUserList').innerHTML = '<div class="small text-muted">No available volunteers to contact.</div>';
                    return;
                }

                document.getElementById('requestVolunteerUserList').innerHTML = candidates.map(u => `
                    <label class="d-flex align-items-center mb-1">
                        <input type="checkbox" class="form-check-input request-volunteer-user-checkbox me-2" value="${u.UserNr}" data-email="${escHtml(u.email)}" checked>
                        ${escHtml(u.FirstName + ' ' + u.LastName)} (${escHtml(u.email)})
                    </label>
                `).join('');
            })
            .catch(() => {
                document.getElementById('requestVolunteerUserList').innerHTML = '<div class="text-danger small">Failed to load volunteers, please try again later.</div>';
            });

        const modal = new bootstrap.Modal(document.getElementById('requestVolunteerModal'));
        modal.show();
        window._currentRequestVolunteerModal = modal;
        window._requestVolunteerPatrolNr = patrolNr;
    }

    async function sendRequestVolunteerEmails() {
        const patrolNr = window._requestVolunteerPatrolNr;
        if (!patrolNr) return;

        const mode = document.getElementById('requestVolunteerMode').value;
        const shortage = Math.max(1, parseInt(document.getElementById('requestVolunteerShortage').value, 10) || 1);
        const note = document.getElementById('requestVolunteerMessage').value.trim();

        const selectedCheckboxes = Array.from(document.querySelectorAll('.request-volunteer-user-checkbox:checked'));
        if (selectedCheckboxes.length === 0) {
            showToast('Please select at least one volunteer.', 'warning');
            return;
        }

        const emails = selectedCheckboxes
            .map(c => c.dataset.email?.trim())
            .filter(email => !!email);

        if (emails.length === 0) {
            showToast('No valid email addresses found, please refresh.', 'danger');
            return;
        }

        document.getElementById('requestVolunteerSubmitBtn').disabled = true;
        document.getElementById('requestVolunteerSpinner').classList.remove('d-none');

        try {
            const res = await fetch(`/api/schedules/${patrolNr}/email-reminder`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
                body: JSON.stringify({ emails, message: note, shortage })
            });
            const data = await res.json();
            if (res.ok && data.success) {
                showToast(`${data.sent} email(s) sent.`, 'success');
                window._currentRequestVolunteerModal?.hide();
            } else {
                showToast(data.message || 'Failed to send emails, please try again.', 'danger');
            }
        } catch (err) {
            showToast('Network error, please try again later.', 'danger');
        } finally {
            document.getElementById('requestVolunteerSubmitBtn').disabled = false;
            document.getElementById('requestVolunteerSpinner').classList.add('d-none');
        }
    }

    function buildFinalizedMembersPanel(patrol, isFinalized, patrolNr) {
        ...
        if (supervisor) {
            if (isManager) {
                ...
                listHtml += `<li>${escHtml(supervisor.name || `User #${supervisor.id}`)}</li>`;
            }
        }
    ...
    function renderUpcomingPatrols() {
        ...
        if (myPatrols.length === 0) {
            container.innerHTML = `<div class="upcoming-empty">
                ...
                No upcoming patrols found. Browse the calendar below to sign up.
            </div>`;
            return;
        }
            ...
            <div class="upcoming-patrol-item">
                ...
                <div class="upcoming-patrol-info">
                    ...
                    <div class="upcoming-patrol-meta">
                        ...
                        ${isManager ? `&nbsp;·&nbsp;<i class="bi bi-person-badge me-1"></i>${superTxt}` : ''}
                    </div>
                </div>
                <div class="flex-shrink-0">
                    <button class="btn btn-sm btn-outline-danger" onclick="cancelSignup(${p.patrolNr})" style="min-height:30px;">
                        <i class="bi bi-x-circle me-1"></i><span class="cancel-text">Cancel</span>
                    </button>
                </div>
            </div>`;
        }).join('');
    ...
    function renderAgendaView() {
        ...
        daysWithPatrols.forEach(dateStr => {
            ...
            dayPatrols.forEach(p => {
                ...
                const isFinalized = p.patrol_status == 2;
                let signupLabel;
                if (isFinalized) {
                    signupLabel = '<span class="badge ms-1" style="background:#dcfce7;color:#166534;font-size:.68rem;"><i class="bi bi-lock me-1"></i>Finalized</span>';
                } else if (isSignedUp) {
    ...
    function updateSelectionPanel() {
        ...
        if (hasPatrols) {
            ...
            <button class="sel-action-btn s-postpone" onclick="openPostponeModal()"><i class="bi bi-clock-history"></i> Postpone</button>
    ...
    function renderPatrolCard(patrol) {
        ...
        const finalizedPanel = buildFinalizedMembersPanel(patrol, isFinalized, nr);
        ...
        const shortfall = Math.max(0, (patrol.min_volunteers || 0) - count);
        const showRecruitBtn = isManager && patrol.patrol_status == 1 && shortfall > 0;
        ...
        <div class="patrol-open-card" id="patrolCard_${nr}">
            <div class="d-flex align-items-start justify-content-between flex-wrap gap-3 mb-2">
                <div>
                    ...
                    ${isManager ? `<div class="small text-muted"><i class="bi bi-person-badge me-1"></i>${superTxt}</div>` : ''}
                </div>
            ...
            <div class="member-count-bar">
                ...
                <span class="member-count-text">
                    ...
                    ${patrol.min_volunteers ? ` • min ${patrol.min_volunteers}` : ''}
                    ${patrol.max_volunteers ? ` • max ${patrol.max_volunteers}` : ''}
                </span>
            </div>
            ${showRecruitBtn ? `<div class="mt-2"><button class="btn btn-sm btn-warning" onclick="openRequestVolunteerModal(${nr}, ${shortfall})"><i class="bi bi-megaphone-fill me-1"></i>Request ${shortfall} more</button></div>` : ''}
            ${finalizedPanel}
        </div>`;
    }
    ...
    function toggleManagerVolunteerMenu() {
        const header = document.getElementById('managerVolHeaderToggle');
        const body = document.getElementById('managerVolunteerBody');
        if (!header || !body) return;
        const collapsed = header.getAttribute('data-collapsed') === 'true';
        header.setAttribute('data-collapsed', !collapsed);
        body.classList.toggle('d-none');
    }

    function toggleMonthlyStats() {
        const header = document.getElementById('statsHeaderToggle');
    ...
    function renderManagerVolunteerSidebar() {
        const container = document.getElementById('managerVolunteerList');
        if (!container || !isManager) return;

        const patrols = allSchedules
            .filter(p => (p.patrol_status == 1 || p.patrol_status == 2 || p.patrol_status == 3))
            .filter(p => {
                const volunteerCount = Array.isArray(p.volunteers) ? p.volunteers.length : 0;
                return volunteerCount > 0 || !!p.supervisor;
            })
            .sort((a, b) => {
                const da = (a.patrolDate || '').slice(0,10);
                const db = (b.patrolDate || '').slice(0,10);
                if (da === db) return Number(a.patrolNr) - Number(b.patrolNr);
                return da.localeCompare(db);
            });

        if (!patrols.length) {
            container.innerHTML = `<div class="manager-vol-empty">
                <i class="bi bi-person-x d-block mb-2 fs-4"></i>
                No assigned team members found in released/finalized patrols.
            </div>`;
            return;
        }
        ...
        container.innerHTML = patrols.map(p => {
            ...
            const dateLabel = dateStr
                ? new Date(`${dateStr}T00:00:00`).toLocaleDateString('en-IE', { day:'numeric', month:'short', year:'numeric' })
                : 'No date';
            const status = p.patrol_status == 2 ? 'Finalized' : (p.patrol_status == 3 ? 'Postponed' : 'Released');
            const volunteerChips = (Array.isArray(p.volunteers) ? p.volunteers : [])
                .map(v => `<span class="manager-vol-chip">${escHtml(v.name || `Volunteer #${v.id}`)}</span>`)
                .join('');
            const supervisorChip = p.supervisor
                ? `<span class="manager-vol-chip">${escHtml(p.supervisor.name || `User #${p.supervisor.id}`)} (SUPER)</span>`
                : '';
            const names = `${volunteerChips}${supervisorChip}`;

            const assignedCount = p.volunteer_count ?? ((Array.isArray(p.volunteers) ? p.volunteers.length : 0));
            const targetCount = p.maxVolunteers ?? 0;
            const shortfall = Math.max(1, targetCount > assignedCount ? targetCount - assignedCount : 1);
            const emailButton = p.patrol_status == 1
                ? `<div class="manager-vol-actions"><button class="btn btn-sm btn-outline-warning" onclick="openRequestVolunteerModal(${p.patrolNr}, ${shortfall})"><i class="bi bi-megaphone-fill me-1"></i>Request Volunteers</button></div>`
                : '';

            return `
            <div class="manager-vol-item">
                <div class="manager-vol-content">
                    <div class="manager-vol-meta">
                        <span><i class="bi bi-shield-check me-1"></i>Patrol #${p.patrolNr}</span>
                        <span><i class="bi bi-calendar3 me-1"></i>${dateLabel}</span>
                        ${statusPill(p.patrol_status)}
                    </div>
                    <div class="manager-vol-names">${names}</div>
                    ${emailButton}
                </div>
            </div>`;
        }).join('');
    ...
    function openPostponeModal() {
        ...
        bootstrap.Modal.getOrCreateInstance(document.getElementById('postponePatrolModal')).show();
    }

    async function submitPostponePatrol() {
        const patrolIds = getSelectedPatrolIds();
        ...
        if (new Date(`${newDate}T00:00:00`) < new Date(new Date().toDateString())) {
            showToast('Cannot postpone to a past date.', 'danger');
            return;
        }

        document.querySelector('#postponePatrolModal .btn-primary').disabled = true;

        const results = await Promise.allSettled(patrolIds.map(id => {
            const patrol = allSchedules.find(p => p.patrolNr == id);
            if (!patrol) return Promise.reject(new Error('Patrol not found'));

            return fetch(`/api/schedules/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    patrolDate: `${newDate} ${(patrol.start_time || '08:00:00').slice(0, 8)}`,
                    patrol_status: 3,
                    postponed: true
                })
            }).then(async r => {
                if (!r.ok) {
                    let msg = 'Failed to postpone patrol.';
                    try {
        ...
        if (failed > 0) {
            ...
            const errMsg = firstErr?.reason?.message ? ` ${firstErr.reason.message}` : '';
            showToast(`${patrolIds.length - failed} postponed, ${failed} failed.${errMsg}`, 'danger');
            document.querySelector('#postponePatrolModal .btn-primary').disabled = false;
        } else {
            showToast(`${patrolIds.length} patrol${patrolIds.length !== 1 ? 's' : ''} postponed to ${newDate}.`, 'success');
            bootstrap.Modal.getInstance(document.getElementById('postponePatrolModal')).hide();
            setTimeout(() => window.location.reload(), 800);
        }
    }

    // -- Manager: Monthly volunteer signup statistics ----
    function renderMonthlyVolunteerStats() {
        if (!isManager) return;
        
        const container = document.getElementById('volunteerStatsContainer');
        const icon = document.querySelector('.stats-toggle-icon');
        if (!container) return;
        ...
        // Get patrols for the current month
        const firstOfMonth = new Date(calYear, calMonth - 1, 1);
        const lastOfMonth  = new Date(calYear, calMonth, 0);

        const monthlyPatrols = allSchedules.filter(p => {
            const dateStr = (p.patrolDate || '').slice(0, 10);
            if (!dateStr) return false;
            const d = new Date(`${dateStr}T00:00:00`);
        ...
        // Initialize stats with all volunteers (count = 0)
        const volunteerStats = new Map(); // userId -> { name, email, count }
        allVolunteers.forEach(vol => {
            volunteerStats.set(vol.UserNr, { 
        ...
        // Count signups for volunteers in monthly patrols
        monthlyPatrols.forEach(patrol => {
            if (Array.isArray(patrol.volunteers)) {
                patrol.volunteers.forEach(vol => {
                    const volId = vol.id;
                    if (volunteerStats.has(volId)) {
                        volunteerStats.get(volId).count++;
                    } else {
                        const volName = vol.name || `Volunteer #${volId}`;
                        volunteerStats.set(volId, { name: volName, email: vol.email || '', count: 1 });
                    }
                });
            }
        });
        ...
        const sortedVols = Array.from(volunteerStats.values())
            .sort((a, b) => {
        ...
        // Update month label
        const monthLabel = new Date(calYear, calMonth - 1, 1).toLocaleDateString('en-IE', { month: 'short', year: 'numeric' });
        document.getElementById('statsMonthLabel').textContent = monthLabel;

        // Render
        if (sortedVols.length === 0) {
            container.innerHTML = `<div class="small text-muted text-center py-3">
        ...
        container.innerHTML = sortedVols.map((vol, idx) => {
            const badgeColor = vol.count === 0 ? 'bg-secondary' : 'bg-success';
            const emailDisplay = vol.email ? `<div class="small text-muted" style="font-size:.7rem;">${escHtml(vol.email)}</div>` : '';
            return `
            <div style="display:flex;align-items:flex-start;padding:8px 0;border-bottom:1px solid #f1f5f9;justify-content:space-between;gap:8px;">
                <div style="flex:1;min-width:0;">
                    <span class="small text-truncate" style="display:block;">${escHtml(vol.name)}</span>
                    ${emailDisplay}
                </div>
                <span class="badge ${badgeColor}" style="font-size:.75rem;flex-shrink:0;">${vol.count}×</span>
            </div>`;
        }).join('');
    }
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        // Load all volunteers for manager stats
        if (isManager) {
            try {
                const res = await fetch('/api/users/volunteers');
                if (res.ok) {
                    const data = await res.json();
        ...
        renderUpcomingPatrols();
        renderManagerVolunteerSidebar();
        renderMonthlyVolunteerStats();
        refreshSmileyIndicators();
